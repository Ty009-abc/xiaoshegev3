...
function buildSystemPrompt () {
  return [
    ...
    'strategicMigration.id 必须复制 allowedTargetPositions[0].id（结构化字段，机器用）。',
    '如果用户的能力还没有产生过任何收入，目标不能直接是“稳定收入 / 可重复收入 / 系统化收入”，',
    '只能是“把它变成一个别人愿意付费的最小交付，先拿到第一笔付费证据”。',
    'commercialHypothesis.class 只能来自 allowedStrategyHypotheses（结构化字段，机器用）。',
    'actionThesis.experimentClass 必须等于信封里的 experimentClass（结构化字段，机器用）。',
    ...
    '"strategicMigration":{"id":"<allowedTargetPositions 里的 id>","from":"...","to":"...","logic":"..."},',
    '"commercialHypothesis":{"class":"<allowedStrategyHypotheses 里的 id>","intent":"...","text":"..."},',
    '"actionThesis":{"experimentClass":"<信封里的 experimentClass>","testTarget":"...","successSignal":"...","text":"..."}},',
    '"cards":{"card01":"...","card02":"...","card03":["...","...","...","..."],"card04":{"from":"...","to":"...","logic":"..."},',
    '"card05":{"primary":"...","supporting":["...","..."],"target":"...","timebox":"...","successSignal":"..."}}}',
    '',
    '注意：strategicMigration.id 必须复制 allowedTargetPositions[0].id；',
    'commercialHypothesis.class 必须从 allowedStrategyHypotheses 里选；',
    'actionThesis.experimentClass 必须等于信封里的 experimentClass。',
    '这三个字段是机器校验用的结构化字段，不属于正文，不要写进面向用户的卡片文案。',
    'card01–card05 里的正文请用自然语言，不要出现任何 id。',
    '',
    '== UNPROVEN 连接验证（重要）==',
    '当 crossAxisScope=UNPROVEN 且 experimentClass=LINK_TEST 时：',
    '第五张卡要验证的是“这项已被付费的能力 ↔ 当前目标”的连接——即“能不能用上 / 连不连得上”。',
    '关键：不要说“扩大/复制/系统化这项能力”，也不要把那次成交直接当成要走的路。',
    '连接验证只要在行动上真的去验证连接就算成立，不要求正文里出现“连接/用得上/目标方向/这条路”这些具体词。',
    '宁可写成一次真实的行为验证（例：用它帮目标方向里的人把一件具体的事往前推一小步，看他是不是真的需要），',
    '也不要只停在“你愿不愿意”的口头询问。'
  ].join('\n')
}
