 ...
 * result UI (Progressive Reveal + Canvas poster + share), kept faithful to the
 * proven online experience.
 *
 * ONLY the report wiring is bridged to the CURRENT Stage2 contract via
 * services/legacy6qReportService.js (`turnaround_strategy_6q_v1`). The adapter
 * maps the Stage2 envelope into the 07/11 UI field names; the result page is
 * NOT redesigned.
 *
 * Visible order: 01 致命一句话 → 02 核心问题 → 03 系统困局 → 04 翻身路径 → 05 行动建议.
 * Progressive reveal: 300 / 700 / 1100 / 1500 / 1900 ms.
 *
...
Page({
  ...
  async _startDiagnostic () {
    ...
    if (!answers) {
      ...
      setTimeout(() => wx.redirectTo({ url: QUESTIONNAIRE_ROUTE + '?mode=diagnostic' }), 1500)
      return
    }

    try {
      const r = await legacy6q.generateLegacy6QReport({
        answers,
      ...
      console.error('[6q-report] AI 生成失败:', e)
  ...
  _showError (msg) {
    ...
    setTimeout(() => wx.redirectTo({ url: QUESTIONNAIRE_ROUTE + '?mode=diagnostic' }), 3000)
  },
